...
const InfoRow = ({
  ...
  >
    ...
    <div className="flex-1 min-w-0">
      <p className="text-[9px] md:text-[11px] font-bold uppercase tracking-widest text-muted-foreground/60 leading-none mb-1">
        {label}
      </p>
    ...
    <div className="flex flex-col gap-2 animate-in fade-in duration-500 h-full max-h-[calc(100dvh-130px)] md:max-h-[calc(100dvh-140px)] overflow-hidden">
        ...
        <div className="flex items-center justify-between bg-primary/5 p-2 rounded-xl border border-primary/20 shadow-sm">
          ...
          {selectedExtendido.stockInicial ? (
            <div className="text-right pr-1">
              <p className="text-[8px] md:text-[9px] font-bold text-muted-foreground uppercase tracking-tighter leading-none mb-0.5">
                Stock
              </p>
      ...
      >
        <TabsList className="grid grid-cols-2 bg-muted/80 p-1 rounded-xl shrink-0 h-10 border border-border/40 gap-1 shadow-inner">
          <TabsTrigger
            value="produccion"
            className="rounded-lg text-[9px] md:text-[11px] font-black uppercase tracking-widest transition-all duration-300
          ...
          <TabsTrigger
            ...
            className="rounded-lg text-[9px] md:text-[11px] font-black uppercase tracking-widest transition-all duration-300
                       ...
                       data-[state=inactive]:text-muted-foreground"
          >
            <ClipboardList className="h-3 w-3 md:h-3.5 md:w-3.5 mr-1 hidden sm:inline-block" />
            Notas
          </TabsTrigger>
        </TabsList>
                  ...
                  <InfoRow
                    ...
                    badge={
                      isToday ? (
                        <Badge
                          variant="outline"
                          className="text-warning border-warning/20 bg-warning/10 font-bold px-1.5 py-0 h-4 md:h-5 text-[9px] md:text-[11px]"
                        >
                          Hoy
                        </Badge>
                      ) : isTomorrow ? (
                        <Badge
                          variant="default"
                          className="bg-primary/10 text-primary border-primary/20 font-bold px-1.5 py-0 h-4 md:h-5 text-[9px] md:text-[11px]"
                        >
                          Mañana
                        </Badge>
                      ) : null
                  ...
                  <div className="relative z-10 flex items-center justify-between">
                    <div className="space-y-2">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <span className="text-[8px] md:text-[11px] font-black uppercase tracking-widest text-muted-foreground/60 block leading-none mb-1">
                            Estadía Sugerida
                          </span>
              ...
              <CardContent className="p-3 space-y-2">
                ...
                {selectedExtendido.detalle ? (
                  <div className="space-y-1.5">
                    ...
                    <div className="p-3 rounded-xl bg-muted/30 border border-border/40 text-xs md:text-sm leading-relaxed text-foreground/70 min-h-[60px]">
                      {selectedExtendido.detalle}
                    </div>
                  </div>
                ) : null}
